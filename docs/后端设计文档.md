...
# 压缩触发的百分比阈值
AUTOCOMPACT_THRESHOLD_PERCENT = 0.8
...
def get_auto_compact_threshold(model: str) -> int:
    """压缩触发阈值 = 有效上下文窗口 * 压缩百分比"""
    return int(get_effective_context_window(model) * AUTOCOMPACT_THRESHOLD_PERCENT)
```
...
async def agent_node(state: AgentState) -> AgentState:
    ...
    model = settings.GEMINI_MODELS.split(",")[0]  # 取配置的第一个模型
